...
\begin{enumerate}
\item The Cat can be only in two states: Sleeping and Eating. 
Cat's mood depends only on the previous state. 
The transition probabilities are given below:

%\begin{minipage}
    \begin{tikzpicture}[ -> , >=stealth , shorten >=2 pt , line width =0.5 pt, node distance =2 cm ]
        \node [circle, draw] (zero) {eating};
        \node [circle, draw] (one) [right of = zero] {sleeping};
        \path (zero) edge [bend left] node [above] {$0.1$} (one) ;
        \path (one) edge [bend left] node [below] {$0.2$} (zero) ;
        \path (zero) edge [loop left] (zero) ;
        \path (one) edge [loop right] (one) ;
    \end{tikzpicture}        
%\end{minipage}
